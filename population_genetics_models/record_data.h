...
namespace record {
  ...
  void generation_trait_extinction(data::Int64List* gen_extinct, const std::vector<double> &trait_freq,
				   const P &parameters, const int gen){
    ...
    } else if (!conditional_existence_status::trait_extinct(trait_freq, parameters)){
      gen_extinct->add_value(parameters.fixed.max_generations_per_sim); // trait persists, denote by max gen
    } else {
      exit(EXIT_FAILURE);
    }
  }
  ...
  void number_reinvasions_before_extinction(data::Int64List* reinvasion_number, const std::vector<double>
					    &trait_freq, const P &parameters, const int reinvasions){
